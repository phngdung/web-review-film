...
export default function UserReviewList(props) {
  ...
  useEffect(() => {
    ...
    fetch("http://localhost:8080/user/" + query + "/reviews")
      .then((resp) => resp.json())
  ...
  return (
    <div>
              ...
              <ListItem button>
                ...
                <Link href={"/movie/" + x.movie_id} className={classes.link}>
                  <Typography className={classes.box}>
                    <Rating readOnly value={parseInt(x.rating)} />
                  </Typography>
                </Link>
                <Link href={"/movie/" + x.movie_id} className={classes.link}>
      ...
      <List component="nav" className={classes.list}>
        {reviews
          ? reviews.people.map((x) => (
              <ListItem button>
                <Link href={"/people/" + x.people_id} className={classes.link}>
                  <img
                    src={
                      x.image
                        ? "https://image.tmdb.org/t/p/w500" + x.image
                        : "https://via.placeholder.com/400x600"
                    }
                    className={classes.poster}
                  />
